...
# or
# start pi inside con-terminal
```

Optional: set `PI_SUBAGENT_MUX=cmux|tmux|zellij|wezterm|con` to force a specific backend.
...
Subagent panes are created without stealing keyboard focus where supported (cmux, tmux). Launch commands target child surfaces by explicit ID, so focus and command delivery are independent. On cmux and con, the first subagent gets a worker split and later subagents from the same Pi process attach as tabs/surfaces inside that worker pane. Note: the `interactive` option controls parent status notifications, not terminal focus.
